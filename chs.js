/*

 @name    : 锅巴汉化 - Web汉化插件
 @author  : 麦子、JAR、小蓝、好阳光的小锅巴
 @version : V0.6.1 - 2019-07-09
 @website : http://www.g8hh.com

*/

//1.汉化杂项
var cnItems = {
    _OTHER_: [],

    //未分类：
    'Save': '保存',
    'Load game': '加载游戏',
    'Export Game': '导出',
    'Import Game': '导入',
    'Autosave': '自动保存',
    'Back': '返回',
    'Building': '建筑',
    'Buy selected': '购买选择',
    'Change color mode': '改变颜色模式',
    'ChangeColorMode': '改变颜色模式',
    'City': '城市',
    'Deutsch': '德语',
    'Dismiss hero': '解雇英雄',
    '1255 Burgomaster': '市长1255',
    'Launch autocampaign!': '启动自动战役!',
    'Later': '晚些时候',
    'Knight': '骑士',
    'How to play': '玩法',
    'How To Play': '玩法',
    'Hiring troops': '雇佣军队',
    'Hiring and managing troops': '雇佣和管理部队',
    'Hiring a guard will cost you 20 gold.': '雇佣一名警卫需要20黄金。',
    'Hire turkopol': '雇佣图尔科波尔',
    'Hire sergeant': '雇佣军士',
    'Hire knight': '雇佣骑士',
    'Hire hero': '雇佣英雄',
    'Hero\'s purse': '英雄的钱包',
    'Gold history tab': '黄金历史记录选项卡',
    'Go to the adventure map': '前往冒险地图',
    'Go to exploring!': '去探索！',
    'Garrison': '驻军',
    'Français': '法语',
    'Fire': '解雇',
    'feedback': '反馈',
    'fountain': '喷泉',
    'Fountain': '喷泉',
    'Gallows': '绞架',
    'gallows': '绞架',
    'Hire': '雇佣',
    'Home': '家',
    'home': '家',
    'Inn': '客栈',
    'knights': '骑士',
    'lines': '线',
    'Main': '主要',
    'Stash': '藏匿',
    'Settings': '设置',
    'Option': '选项',
    'Hero for hire': '雇佣英雄',
    'Get high chances to increase ATK when level up': '升级时有很高的机会增加攻击力',
    'Autocampaign battles': '自动战斗',
    'ATK:2, DEF:2, INT: 2, MP: 20': '攻击:2, 防御:2, 智力: 2, 蓝: 20',
    'Music all': '所有音乐',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    'Mobile UI': '手机界面',
    'Next': '下一个',
    'Normally, each citizen pays to you one gold piece per tick.': '通常，每位公民每tick向您支付一枚金币。',
    'the hero\'s purse': '英雄的钱包',
    'Stable': '稳定',
    'Sell selected': '出售已选',
    'Save game': '保存游戏',
    'Regenerate map': '重新生成地图',
    'though parts of the game or game in whole could be provided in the CSL.': '尽管游戏的一部分或全部游戏都可以在CSL中提供。',
    'This game has some clock under the hood. Each clock\'s tick happens in 30 seconds.': '这个游戏的引擎盖下有个时钟。每个时钟的tick发生在30秒内。',
    'The blackmarket\'s merchant offers these goods for sale!': '黑市商人把这些东西拿来卖!',
    'The blackmarket\'s merchant could buy your goods.': '黑市商人可能会买你的货。',
    'Teleport to the town!': '传送到城镇!',
    'Supported Languages(SL): English, Russian (Русский).': '支持的语言(SL):英语、俄语(Русский)。',
    'Stash is required to lower your losses from stealing. You cannot build Stash without Treasury.': '需要储藏来降低偷窃造成的损失。没有国库，你无法建立仓库。',
    'Sound effects and music': '音效和音乐',
    'Send hero to autocampaign': '发送英雄到自动战役',
    'or parts of translation which are partly outdated.': '或部分过时的翻译部分。',
    'Open sound settings': '打开声音设置',
    'Open Leaderboard': '打开排行榜',
    'Open journal': '打开日记',
    'Put out the fire': '把火灭了',
    'It is also increase taxes and decrease birthrate and happiness.': '它还增加了税收，降低了出生率和幸福感',
    'It is also increase birthrate and happiness and decrease taxes.': '它还可以提高出生率和幸福感，并减少税收。',
    'If you do not have enough money, they will leave their work permanently.': '如果您没有足够的钱，他们将永久离开他们的工作。',
    'If you build this building, you will not be able to build the Gallows!': '如果您建造这座建筑，您将无法建造绞刑架！',
    'If you build this building, you will not be able to build the Fountain!': '如果您建造此建筑物，则将无法建造喷泉！',
    'Hero now is training in town': '英雄现在在城里训练',
    'Hero now is resting in town': '英雄现在在城里休息',
    'Hero now is fistfighting in town': '英雄现在在城里打架',
    'Hero now is eating in town': '英雄现在在城里吃东西',
    'Hero now is drinking in town': '英雄现在在城里喝酒',
    'Archery range': '射箭场',
    'Your nickname': '你的昵称',
    'You need to build Walls and Tower (Keep) to keep your city safe against intruders.': '你需要建造城墙和塔楼(保持)来保护你的城市免受入侵。',
    'Welcome to Inn!': '欢迎来到客栈！',
    'Wall': '墙',
    'walls': '墙',
    'Use towngate scroll': '使用城门卷轴',
    'Treasury guards significantly decrease chance of gold being stolen from your treasury.': '金库守卫会显著降低你的金库被偷走黄金的几率。',
    'treasury': '国库',
    'Treasury': '国库',
    'Train brother-sergeant': '训练兄弟军士',
    'brother-sergeants': '兄弟军士',
    'Train brother-knight': '训练兄弟骑士',
    'brother-knights': '兄弟骑士',
    'Towngate scroll': '城门卷轴',
    'To the end': '到最后',
    'To the current': '到目前',
    'To the beginning': '回到起初',
    'Download game for playing off-grid (to play without Internet connection': '下载游戏离线玩(在没有互联网连接的情况下玩)',
    'Each guard requires regular payment (each tick), it also costs 10 gold.': '每个守卫需要定期付费(每个周期)，它也需要10黄金。',
    'Each lvl of Houses allows you to have more population.': '每一层的房子可以让你拥有更多的人口。',
    'Each lvl of the treasury allows you to store more gold.': '每一级的国库可以让你储存更多的黄金。',
    'Earlier': '早些时候',
    'Effects on action required events': '对行动所需的事件的影响',
    'Effects on all events': '对所有事件的影响',
    'English': '英语',
    'Esperanto': '世界语',
    'Event log size': '事件日志大小',
    'Execute somebody': '处决某人',
    'Execute smb': '处决某人',
    'Executing any townsperson will add his property as confiscated gold.': '处决任何市民都会把他的财产加为没收的黄金。',
    'Fire brigade': '消防队',
    'fire in the city!': '城市着火了!',
    'Fountains will increase happiness and speed of population growth': '喷泉将增加人口的幸福感和增长速度',
    'Gallows will decrease population, increase taxes, and allow you to execute townsfolk.': '绞刑架将减少人口，增加税收，并允许你处决城镇居民。',
    'Lore and scenery music': '传说与风景音乐',
    'Lore and script music': '传说和剧本音乐',
    'CSL means that the author doesn\'t provide any help or answers asked in the CSL,': 'CSL是指作者在CSL中不提供任何帮助或答案，',
    'Community Supported Languages(CSL): German (Deutsch), Esperanto, French (le français).': '社区支持的语言(CSL):德语(Deutsch)、世界语(Esperanto)、法语(le français)。',
    'Also, Treasury allows you to hire treasury guards.': '同时，国库允许你雇佣国库守卫。',
    'Also, any CSL translation could have mistakes or untranslated parts': '此外，任何CSL翻译都可能有错误或未翻译的部分',
    'All sound effects': '所有音效',
    'All music': '所有音乐',
    'After each clock turn your population is paying taxes to you and growing.': '每过一个赛季，你的人口都在向你交税，而且还在增长。',
    'You can build Fountain OR Gallows. You cannot change your choice after building is done.': '你可以建造喷泉或绞刑架。在建造完成后，您不能更改您的选择。',
    'You could help with translations for the game here': '你可以在这里帮助翻译游戏',
    'You need to build Homes to continue growth of your city population.': '你需要建造房屋来维持城市人口的增长。',
    'You need to build a Treasury to store your gold.': '你需要建立一个宝库来储存你的黄金。',

    //原样
    '': '',
    '': '',
    'turkopols': 'turkopols',

}


//需处理的前缀
var cnPrefix = {
    "(-": "(-",
    "(+": "(+",
    "(": "(",
    "-": "-",
    "+": "+",
    " ": " ",
    ": ": "： ",
    "\n": "",
    "                   ": "",
    "                  ": "",
    "                 ": "",
    "                ": "",
    "               ": "",
    "              ": "",
    "             ": "",
    "            ": "",
    "           ": "",
    "          ": "",
    "         ": "",
    "        ": "",
    "       ": "",
    "      ": "",
    "     ": "",
    "    ": "",
    "   ": "",
    "  ": "",
    " ": "",
    "Build ": "建造",
    "Def.: ": "防御: ",
    "HP: ": "生命值: ",
    "Dmg: ": "伤害: ",
    "Atk.: ": "攻击: ",
    "Alignment: Lawful Neutral (": "结盟:合法中立(",
    "Building costs ": "建筑成本 ",
    "Class: Knight | Level: ": "职业: 骑士 | 等级: ",
    "Intelligence: ": "智力：",
    "lvl ": "等级 ",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
}

//需处理的后缀
var cnPostfix = {
    ":": "：",
    "：": "：",
    ": ": "： ",
    "： ": "： ",
    "/s)": "/s)",
    "/s": "/s",
    ")": ")",
    "%": "%",
    "                   ": "",
    "                  ": "",
    "                 ": "",
    "                ": "",
    "               ": "",
    "              ": "",
    "             ": "",
    "            ": "",
    "           ": "",
    "          ": "",
    "         ": "",
    "        ": "",
    "       ": "",
    "      ": "",
    "     ": "",
    "    ": "",
    "   ": "",
    "  ": "",
    " ": " ",
    "\n": "",
    " of fire extinguished": " 的火被扑灭",
    "population is increased": "人口增加了",
    "amount of money is increased": "金额数量增加了",
    "Your task is simple - manage the city as mayor of the city!": "您的任务很简单-以城市市长的身份管理城市！",
    "Hello player in this incremental game!": "你好，这个增量游戏的玩家!",
    "Citizen will pay taxes each season (30 secs) and population will grow each season": "公民将在每个赛季(30秒)纳税，人口将在每个赛季增长",
    " gold": " 黄金",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
}

//需排除的，正则匹配
var cnExcludeWhole = [
    /^\s*$/, //纯空格
];
var cnExcludePostfix = [
]

//正则替换，带数字的固定格式句子
//纯数字：(\d+)
//逗号：([\d\.,]+)
//小数点：([\d\.]+)
//原样输出的字段：(.+)
var cnRegReplace = new Map([
    [/^requires ([\d\.]+) more research points$/, '需要$1个研究点'],
    [/^Attack: (\d+) \| Defense: (\d+)$/, '攻击: $1 \| 防御: $2'],
    [/^Current experience: (\d+) \| Experience required for next level: (\d+)$/, '当前经验：$1 \| 下一级别要求的经验：$2'],
    [/^Usages: (\d+)\/$/, '用途：$1\/'],
    [/^workers: (\d+)\/$/, '工人：$1\/'],

]);